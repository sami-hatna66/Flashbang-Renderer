...
class AccelerationStructure : public Shape {
    ...
    NodeIndex binning(Node &node, int splitAxis) {
        ...
        // Find region bounds
        float min_bound = 1e30f, max_bound = -1e30f;
        for (int i = 0; i < node.primitiveCount; i++) {
            auto primitive = m_primitiveIndices[node.leftFirst + i];
            min_bound = min(min_bound, getCentroid(primitive)[splitAxis]);
            max_bound = max(max_bound, getCentroid(primitive)[splitAxis]);
        }
        // Fill bins with primitives
        std::array<Bin, NumBins> bins;
        float scale = NumBins / (max_bound - min_bound);
        for (int i = 0; i < node.primitiveCount; i++) {
            auto primitive = m_primitiveIndices[node.leftFirst + i];
            int binIdx = min(NumBins - 1, 
                             max(0, static_cast<int>((getCentroid(primitive)[splitAxis] - min_bound) * scale)));
            bins[binIdx].primitiveCount++;
            bins[binIdx].bounds.extend(getBoundingBox(primitive));
        }
        // Get bin data, used for evaluating cost
        std::array<float, NumBins - 1> leftArea, rightArea;
        std::array<int, NumBins - 1> leftCount, rightCount;
        ...
        // Evaluate bin cost and choose best split position
        scale = (max_bound - min_bound) / NumBins;
        float splitPos = 0;
        for (int i = 0; i < NumBins - 1; i++) {
            float planeCost = leftCount[i] * leftArea[i] + rightCount[i] * rightArea[i];
            if (planeCost < best_cost) {
                splitPos = min_bound + scale * (i + 1.f);
                best_cost = planeCost;
            }
        }
        // Partition according to best split position
        int i = node.leftFirst;
        int j = i + node.primitiveCount - 1;
        while (i <= j) {
            if (getCentroid(m_primitiveIndices[i])[splitAxis] < splitPos) i++;
            else std::swap(m_primitiveIndices[i], m_primitiveIndices[j--]);
        }
